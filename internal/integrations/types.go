	...
	return []spec{
		{
			...
			[]string{"pi"},
			piDir,
			func() string { return filepath.Join(piDir(), "extensions", "seshagy-agent-state.ts") },
			installPi,
			uninstallPi,
		},
		{
			TargetClaude,
			"Claude Code",
			[]string{"claude", "claude-code"},
			claudeDir,
			func() string { return filepath.Join(claudeDir(), "hooks", shellHookName) },
			installClaude,
			uninstallClaude,
		},
		{
			...
			[]string{"codex"},
			codexDir,
			func() string { return filepath.Join(codexDir(), shellHookName) },
			installCodex,
			uninstallCodex,
		},
		{
			TargetCopilot,
			"GitHub Copilot CLI",
			[]string{"copilot", "github-copilot", "ghcs"},
			copilotDir,
			func() string { return filepath.Join(copilotDir(), "hooks", shellHookName) },
			installCopilot,
			uninstallCopilot,
		},
		{
			...
			[]string{"droid"},
			droidDir,
			func() string { return filepath.Join(droidDir(), "hooks", shellHookName) },
			installDroid,
			uninstallDroid,
		},
		{
			TargetOpencode,
			"OpenCode",
			[]string{"opencode", "open-code"},
			opencodeDir,
			func() string { return filepath.Join(opencodeDir(), "plugins", "seshagy-agent-state.js") },
			installOpencode,
			uninstallOpencode,
		},
		{
			TargetQodercli,
			"Qoder CLI",
			[]string{"qodercli", "qoder", "qoderclicn", "qodercn"},
			qoderDir,
			func() string { return filepath.Join(qoderDir(), "hooks", shellHookName) },
			installQodercli,
			uninstallQodercli,
		},
		{
